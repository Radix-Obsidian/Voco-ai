...
export function FloatingOrb() {
  ...
  return (
    <div className="fixed inset-0" style={{ background: "transparent", pointerEvents: "none" }}>
      ...
      >
        {/* The Orb — entire area is draggable + clickable */}
        <div
          onMouseDown={handleMouseDown}
          className="relative flex items-center justify-center w-[48px] h-[48px] cursor-grab active:cursor-grabbing"
        >
          <button
            onClick={handleClick}
        ...
        {state.liveTranscript && state.isListening && (
          <div className="absolute bottom-full mb-1 left-1/2 -translate-x-1/2 whitespace-nowrap max-w-[180px] truncate px-2 py-1 rounded-md bg-black/90 text-[10px] text-zinc-300 border border-white/[0.06]">
        ...
        {showMenu && (
          <div
            ref={menuRef}
            className="absolute bottom-full mb-1 right-0 min-w-[130px] bg-[#1a1a1a] border border-white/[0.08] rounded-lg shadow-xl overflow-hidden z-50"
          >
            <button
